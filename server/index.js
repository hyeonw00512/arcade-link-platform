...
import { PostgresStateStore } from './platform/storage/postgres-state-store.js';
...
const stateStore = process.env.DATABASE_URL ? new PostgresStateStore() : new FileStateStore();
const restoredState = await stateStore.load();
let sessions;
let rooms;
const persistState = () => Promise.resolve(stateStore.save({ sessions: sessions.exportState(), rooms: rooms.exportState() }))
  .catch((error) => console.error('플랫폼 상태 저장에 실패했습니다.', error));
